...
@interface FPopNetworkInterfaceStatus : NSObject {
    NSString *interface;
    NSDictionary *status;
...
@implementation FPopNetworkInterfaceStatus
...
@implementation FPopStatusNetworkMonitor
...
-(void)updateInterface:(NSString*)interface forType:(NetworkInterfaceType)type withStatus:(NSDictionary*)status {
    FPopNetworkInterfaceStatus *new = [[[FPopNetworkInterfaceStatus alloc] initForInterface:interface
                                                                                           ofType:type
...
-(void)updateAirportWithInterface:(FPopNetworkInterfaceStatus*)interface {
    NSString *interfaceString = [interface interface];
    NSDictionary *newValue = [interface status];
    NSDictionary *existing = [(FPopNetworkInterfaceStatus*)[networkInterfaceStates objectForKey:interfaceString] status];
...
-(void)updateLinkWithInterface:(FPopNetworkInterfaceStatus*)interface {
    NSString *interfaceString = [interface interface];
    NSDictionary *newValue = [interface status];
    NSDictionary *existing = [(FPopNetworkInterfaceStatus*)[networkInterfaceStates objectForKey:interfaceString] status];
...
static void scCallback(SCDynamicStoreRef store, CFArrayRef changedKeys, void *info) {
    @autoreleasepool {
        FPopStatusNetworkMonitor *observer = info;
